# Contributing to ep_rewrite_share_paths

Thanks for helping improve `ep_rewrite_share_paths` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_rewrite_share_paths` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_rewrite_share_paths from your local clone
pnpm run plugins i --path ../ep_rewrite_share_paths

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_rewrite_share_paths/` are picked up after a server restart.
...
* Etherpad version, ep_rewrite_share_paths version, browser + OS
* Steps to reproduce
